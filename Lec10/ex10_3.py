...
if len(sys.argv) != 3:
    sys.exit("Usage: ex10_3.py <input fasta file> <k>")
...
# for kmer in itertools.product("atcg", repeat=k):
#     kmer = "".join(kmer)
#     tot_kmer_dict[kmer] = sum([kdict.get(kmer.encode(), 0) for kdict in kmer_dict_list])
#
# tot_nt = sum(tot_nt_count)
# tot_kmer = sum(tot_kmer_dict.values())
#
# Pnt_dict = dict()
